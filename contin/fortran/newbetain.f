      ...
      LOGICAL SWAP
      double precision x1,a1,b1,prob
      DIMENSION IHOLER(6)
      DATA IHOLER/1HB, 1HE, 1HT, 1HA, 1HI, 1HN/, TOL/1.E-5/
C      write(*,*)'amadi0 ','X= ',X,A,'B= ',B
      IF (X.LT.0. .OR. X.GT.1. .OR. AMIN1(A,B).LE.0. .OR.
     1 AMAX1(A,B).GE.2E+5) CALL ERRMES (1,.TRUE.,IHOLER,NOUT)
      x1=x
      a1=a
      b1=b
      CALL MDBETA ( x1,a1,b1,prob, IER )      
       BETAIN=PROB
C      write(*,*)'amadi1 ','X= ',X,'A= ',A,'B= ',B,'BETAIN= ',BETAIN
      RETURN
      END       

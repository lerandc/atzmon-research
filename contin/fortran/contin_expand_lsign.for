...
C-----------------------------------------------------------------------    1773
     ...
     1 ISTAR/1H , 1H*, 1HX, 1HF/                                            1808
      WRITE(*,*) "Entered subroutine ANPEAK"
      NSGNM1=NSGNI-1                                                        1809
      DONE=.FALSE.                                                          1810
      FLAT=.FALSE.                                                          1811
      NDONE=0                                                               1812
...
C-----------------------------------------------------------------------    1855
      DO 229 I = 1,10
        WRITE(*,*) "ANPEAK LLSIGN", I, LLSIGN(I)
  229 CONTINUE
...
C-----------------------------------------------------------------------    4899
     ...
     1 SRANGE)                                                              4901
      ...
      IF (NSGNI.LT.1 .OR. NSGNI.GT.9 .OR. IABS(LSIGN(1,INSGN)).NE.1)        4906
     1 CALL ERRMES (1,.TRUE.,IHOLER,NOUT)                                   4907
      ...
      DO 110 ISGN=2,NSGNI                                                   4911
        ...
        WRITE(*,*) "SETSGN LLSIGN", ISGN, LLSIGN(ISGN)
...
C      (LSDONE(NDONE,J,1),LSDONE(NDONE,J,2)).)                              5443
     ...
     1 VDONE(MDONE)                                                         5449
      ...
      WRITE(*,*) "Entered subroutine UPDDON"
...
C-----------------------------------------------------------------------    5503
     ...
     1 LLSIGN(5), INC(NSGNI)                                                5508
      ...
      WRITE(*,*) "Entered subroutine UPDLLS"
      IF (NSGNI .LE. 1) GO TO 790                                           5511
      WRITE(*,*) "UPDLLS NSGNI", NSGNI
      DO 228 I = 1,NSGNI
        WRITE(*,*) "UPDLLS JSTAGE(L)", I, JSTAGE(I)
  228 CONTINUE
      NSGNP1=NSGNI+1                                                        5512
      DO 235 J = 1,6
      DO 234 ITERA = 1,NSGNP1
        WRITE(*,*) "Initial LLSTRY", ITERA, J, LLSTRY(ITERA,J)
  234 CONTINUE
  235 CONTINUE
      DONE=.FALSE.                                                          5513
      L=NSGNP1                                                              5514
...
C-----------------------------------------------------------------------    5526
        ...
        DO 227 ITER = 1,NSGNI
          WRITE(*,*) "UPDLLS JSTAGE(L) 2", ITER, JSTAGE(ITER)
  227   CONTINUE
        WRITE(*,*) "L value post 227", L
        IF (L .LT. NSGNP1) WRITE(*,*) "One"
...
C-----------------------------------------------------------------------    5534
  ...
  310   CONTINUE                                                            5538
        DO 229 I = 1,NSGNP1
          WRITE(*,*) "UPDLLS 300 BLOCK", I, L, LLSTRY(I,L)
  229 CONTINUE
        GO TO 350                                                           5539
C-----------------------------------------------------------------------    5540
...
C-----------------------------------------------------------------------    5543
  ...
  340   CONTINUE                                                            5547
        DO 222 I = 1,NSGNP1
          WRITE(*,*) "UPDLLS POST 340", I, L, LLSTRY(I,L)
  222 CONTINUE
C-----------------------------------------------------------------------    5548
...
C-----------------------------------------------------------------------    5551
  350   DO 360 LL=2,NSGNI                                                   5552
          WRITE(*,*) "LLSTRY IN 350", LLSTRY(LL,L)
          LLSIGN(LL)=ISIGN(LLSTRY(LL,L),LLSIGN(LL))                         5553
  360   CONTINUE                                                            5554
      DO 243 I = 1,NSGNP1
          WRITE(*,*) "UPDLLS POST 350 LLSTRY", I, L, LLSTRY(I,L)
          WRITE(*,*) "UPDLLS POST 350 LLSGN", I, L, LLSIGN(I)
          WRITE(*,*) "UPDLLS POST 350 LL", I, L, LL
  243 CONTINUE
        LL=LLSTRY(L,L)+INC(L)                                               5555
        LLSIGN(L)=ISIGN(LL,LLSIGN(L))                                       5556
        DO 230 I = 1,NSGNP1
          WRITE(*,*) "UPDLLS POST 360 LLSTRY", I, L, LLSTRY(I,L)
          WRITE(*,*) "UPDLLS POST 360 LLSIGN", I, L, LLSIGN(I)
          WRITE(*,*) "UPDLLS POST 360 LL", I, L, LL
  230 CONTINUE
C                                                                           5557
...
C-----------------------------------------------------------------------    5636
     ...
     1 RHSNEQ(MINEQ), VALPHA(MG), IISIGN(MG), AINEQ(MINEQ,MG)               5643
      ...
      WRITE(*,*) "Entered subroutine UPDSGN"
C     ZERO=0.E0!SP                                                          5646
      ZERO=0.D0                                                             5647
C     ONE=1.E0!SP                                                           5648
      ONE=1.D0                                                              5649

...
for iter = 1:length(files)
    ...
    for N_peaks = 1:7
        ...
        xlswrite(filename,{name},strcat('A',num2str(row),':A',num2str(row)));
            row = row +1;
        xlswrite(filename,M.Properties.VariableNames,strcat('A',num2str(row),':H',num2str(row)));
            row = row +1;    
        xlswrite(filename,table2array(M),strcat('A',num2str(row),':H',num2str(row)));
            row = row +1;
        xlswrite(filename,{'N_peaks',N_peaks},strcat('A',num2str(row),':B',num2str(row)))
            row = row +1;
        xlswrite(filename,{'sse','R^2','dfe','adj. R^2','RMSE';...
                gof.sse,gof.rsquare,gof.dfe,gof.adjrsquare,gof.rmse},...
                strcat('A',num2str(row),':E',num2str(row+1)))
                row = row +2;
        xlswrite(filename,{'A_m','C_m','tau_m','Area'},strcat('A',num2str(row),':D',num2str(row)))
            row = row +1;
        xlswrite(filename,[coef areas],strcat('A',num2str(row),':D',num2str(row-1+size(coef,1))));
